...
class Travel extends Component {
  ...
  constructor(props) {
    ...
    //   this.setState({
    //     Tablelist: [
    //       { KhodroModelCompanyID: 1, CompanyTitle: 'البرز', TedadShoab: '5', NerkhPayeStr: '10200', NerKhKolPlusArzashaafzoodeStr: '20500' },
    //       { KhodroModelCompanyID: 2, CompanyTitle: 'آسیا', TedadShoab: '5', NerkhPayeStr: '10200', NerKhKolPlusArzashaafzoodeStr: '20500' },
    //     ]
    //   });
    // }
    // this.handleTablelist = (Tablelist) => {
    ...
    //   return <tr key={key}>
    //     <td>
    //       <a onclick="edit({{index}})">سفارش</a>
    //     </td>
    //     <td>{this.state.Tablelist[index].CompanyTitle}</td>
    //     <td>{this.state.Tablelist[index].TedadShoab}</td>
    //     <td>{this.state.Tablelist[index].NerkhPayeStr}</td>
    //     <td>{this.state.Tablelist[index].NerKhKolPlusArzashaafzoodeStr}</td>
    //   </tr>;
    //   //<div key={key}>{this.state.Tablelist[index].name}</div>;
    // }
    ...
    this.getTarefe = () => {
      const vm = {
        CountryID: this.state.selectedCountry.CountryID,
      ...
      console.log(vm);
      // axios.post('http://localhost:47002/Home/TarefeTravelCompanyTb_List', vm)
      //   .then((response) => {
      //     const TablelistCur=[];
      //     //const Countryoptions = response.data.map(item => ({ value: item.CountryID, label: item.CountryTitle }));
      //     if (TablelistCur.length > 0) {
      //       this.setState({ TablelistCur:  response.data });
      //     }
      //     console.log('Tablelist', TablelistCur);
      //   })
    ...
    this.getCountryOptions = (CountryStatus) => {
      axios.post('http://localhost:47002/Home/Country_List', { CountryStatus: CountryStatus })
        .then((response) => {
          const Countryoptions = response.data.map(item => ({ value: item.CountryID, label: item.CountryTitle }));
    ...
    this.getTravelAgeoptions = () => {
      axios.post('http://localhost:47002/Home/TravelAge_List', {})
        .then((response) => {
          const TravelAgeoptions = response.data.map(item => ({ value: item.TravelAgeID, label: item.TravelAgeTitle }));
    ...
    this.getTravelDurationoptions = () => {
      axios.post('http://localhost:47002/Home/TravelDuration_List', {})
        .then((response) => {
          const TravelDurationoptions = response.data.map(item => ({ value: item.TravelDurationID, label: item.TravelDurationTitle }));
    ...
    return (
      <div>
        ...
        <div className="col-md-12">
          ...
          <div id="CompanyBime">
            <h3>شرکت های بیمه</h3>
            {/* <BootstrapTable data={this.state.Tablelist} striped hover>
              <TableHeaderColumn isKey dataField='KhodroModelCompanyID'>#</TableHeaderColumn>
              <TableHeaderColumn dataField='CompanyTitle'>نام شرکت</TableHeaderColumn>
              <TableHeaderColumn dataField='NerkhPayeStr'>تعرفه</TableHeaderColumn>
            </BootstrapTable>, */}
            <BootstrapTable data={this.state.Tablelist} version='4' bordered={false} options={{ noDataText: 'موردی یافت نشد' }}>
              <TableHeaderColumn width='50' isKey dataField='KhodroModelCompanyID'>#</TableHeaderColumn>
              <TableHeaderColumn dataField='CompanyTitle'>نام شرکت</TableHeaderColumn>
              <TableHeaderColumn width='150' dataField='NerkhPayeStr'>تعرفه</TableHeaderColumn>
            </BootstrapTable>
            {/* <table id="tbFormRows" className="table table-bordered mb0">
              <thead>
                <tr>
                  <th width="50px">#</th>
                  <th>نام شرکت</th>
                  <th>تعداد شعب شرکت</th>
                  <th>تعرفه </th>
                  <th>نرخ کل</th>
                </tr>
              </thead>
        ...
        <div className="container1 col-md-12">
